...
def load_dataset_qpos(path='/data/tyn/so100_grasp/data/chunk-000/episode_000001.parquet'):
    df = pd.read_parquet(path)
    return df['observation.state'].to_list()

to_rad = lambda x: x * 180 / math.pi
...
init_qpos = np.array([ 0.26367188 ,184.7461  ,   100.634766 ,   99.66797    , -2.109375,   9.083263 ])*3.14159/180
print(init_qpos)
lock = threading.Lock()
